...
-- integration between notebook.nvim and magma

-- safely require notebook (plugin might not be loaded yet)
local ok, notebook = pcall(require, "notebook")
if not ok then
  return -- stop here if plugin not loaded, avoids startup errors
end
...
vim.api.nvim_create_autocmd("User", {
  pattern = { "MagmaInitPost", "NBPostRender" },
